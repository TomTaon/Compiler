...
# handlowanie skokow, optymalizacja
...
def error_loop_initials(iterator, start, stop, lineno1, lineno2):
	if start[0] == "var":
		if start[1] == iterator:
			raise Exception("Błąd w linii " + lineno1 + ": użycie niezadeklarowanej zmiennej " + start[1] + " o tej samej nazwie co iterator w zakresie petli ")
	if stop[0] == "var":
		if stop[1] == iterator:
			raise Exception("Błąd w linii " + lineno2 + ": użycie niezadeklarowanej zmiennej " + stop[1] + " o tej samej nazwie co iterator w zakresie petli ")
